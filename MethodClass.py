...
class SituationMethods(FundamentalMethods):
    ...
    def briefSummary(self,Indicator,level,name='null'):
        ...
        if name == 'null':        
            ...
            print(self.indicatorPrint[Indicator] + 'on the' + level + 'level' + 'is' + totalSum)
            
        else:
            totalSum = df.sum(axis = 0)
            print(self.indicatorPrint[Indicator] + 'in' + name + 'is' + totalSum)
    ...
    def TopRank(self, Indicator, level, name='null'):    
        '''
        ...
        if name == 'null'
            ...
            sortedFrame = rowSumFrame.sort(columns = self.Indicator[Indicator], asending =False)
            if len(sortedFrame.index >= 10):
             ...
             sortedFrame = df.sort(columns = name, asending = False)
             if len(df.index >= 10):
                print(sortedFrame.head(n = 10))
             else:
                 print(sortedFrame)
    ...
    def BoroughCompare(self, Indicator, level, name = 'null'):
        ...
        sumTime.plot(kind = 'bar')
        plt.title('Borough Comparision')
        plt.ylabel(self.IndicatorPrint[Indicator])
        plt.xlable('Boroughs')
        plt.savefig('Borough_comp_for' + self.Indicator[Indicator])
        plt.show()
